...
const int iter                                  = 1e7;    //Number of steps given by the crawling Cells
...
void open_output_files(void)
{
  ...
  files[1] = 0;   //Set files[1]=1 if you want the program to generate and output to a file all the cells' MSD's
  if(files[1]==1)
    {
  ...
  files[4] = 1;    //Set files[4]=1 if you want the program to generate and output to a file all the cells' Mean Speed
  if(files[4]==1)
    {
...
void cell_dynamics (int i, int j)
{
  ...
  dx = vpar[i][j]*dt*px[i][j] + sqrt(q)*xitheta*nx_old;
  dy = vpar[i][j]*dt*py[i][j] + sqrt(q)*xitheta*ny_old;
